...
public class EbonySapling extends BlockBush implements IGrowable, IMetaName, IHasModel
{	
    ...
    public EbonySapling(String name) 
    {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, EbonyTreeEnumHandler.EnumType.EBONY).withProperty(STAGE, Integer.valueOf(0)));
	...
	{
		return this.getDefaultState().withProperty(VARIANT, EbonyTreeEnumHandler.EnumType.byMetadata(meta & 1)).withProperty(STAGE, Integer.valueOf((meta & 8) >> 3));
	}
	...
	public void registerModels() 
	{
		...
		{
			Main.proxy.registerVariantRenderer(Item.getItemFromBlock(this), i, "ebony_sapling" + "_" + EbonyTreeEnumHandler.EnumType.values()[i].getName(), "inventory");
		}
	}
	...
	public void generateTree(World world, Random rand, BlockPos pos, IBlockState state)
	{
		...
		switch((EbonyTreeEnumHandler.EnumType)state.getValue(VARIANT))
		{
		case EBONY:
			gen = new MyrtleEbonyTree();
			break;
		case SKYRIDGE:
			gen = new SkyridgeEbonyTree();
			break;
		}
